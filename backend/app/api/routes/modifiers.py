"""
Modifiers — pricing chain write endpoint.

  GET    /                                      → flat list of modifiers
  PUT    /{modifier_id}/size-pricing/{group_id} → MODIFIER_SIZE_PRICING_SET
  POST   /{modifier_id}/micromods/{micromod_id} → MICROMOD_ASSIGNED_TO_MODIFIER
  DELETE /{modifier_id}/micromods/{micromod_id} → MICROMOD_UNASSIGNED_FROM_MODIFIER
"""

from typing import Dict

from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel

from app.api.dependencies import get_ledger
from app.core import events as evt
from app.core.events import EventType, create_event
from app.core.menu_projection import project_menu

router = APIRouter(prefix="/modifiers", tags=["modifiers"])


class _SizePricingBody(BaseModel):
    size_prices: Dict[str, float] = {}


async def _get_modifier(modifier_id: str, ledger):
    menu = project_menu(await ledger.get_events_since(0, limit=10000))
    mod = menu.modifiers.get(modifier_id)
    if mod is None:
        raise HTTPException(status_code=404, detail=f"Modifier '{modifier_id}' not found")
    return mod


@router.get("", status_code=200)
async def list_modifiers(ledger=Depends(get_ledger)):
    """Flat list of every modifier projected from the ledger."""
    menu = project_menu(await ledger.get_events_since(0, limit=10000))
    return list(menu.modifiers.values())


@router.put("/{modifier_id}/size-pricing/{group_id}", status_code=200)
async def set_modifier_size_pricing(
    modifier_id: str,
    group_id: str,
    body: _SizePricingBody,
    ledger=Depends(get_ledger),
):
    """Set size-aware price adjustments for one modifier relative to a drives_pricing group."""
    await _get_modifier(modifier_id, ledger)
    await ledger.emit(
        evt.EventType.MODIFIER_SIZE_PRICING_SET,
        {
            "modifier_id": modifier_id,
            "group_id": group_id,
            "size_prices": {k: float(v) for k, v in body.size_prices.items()},
        },
        terminal_id="overseer",
    )
    menu = project_menu(await ledger.get_events_since(0, limit=10000))
    return menu.modifiers.get(modifier_id, {})


@router.post("/{modifier_id}/micromods/{micromod_id}", status_code=200)
async def assign_micromod_to_modifier(
    modifier_id: str,
    micromod_id: str,
    ledger=Depends(get_ledger),
):
    """Assign a micromod to a modifier (idempotent)."""
    await _get_modifier(modifier_id, ledger)
    event = create_event(
        event_type=EventType.MICROMOD_ASSIGNED_TO_MODIFIER,
        terminal_id="OVERSEER",
        payload={"modifier_id": modifier_id, "micromod_id": micromod_id},
    )
    await ledger.append(event)
    return {"status": "ok", "modifier_id": modifier_id, "micromod_id": micromod_id}


@router.delete("/{modifier_id}/micromods/{micromod_id}", status_code=200)
async def unassign_micromod_from_modifier(
    modifier_id: str,
    micromod_id: str,
    ledger=Depends(get_ledger),
):
    """Unassign a micromod from a modifier (idempotent if absent)."""
    await _get_modifier(modifier_id, ledger)
    event = create_event(
        event_type=EventType.MICROMOD_UNASSIGNED_FROM_MODIFIER,
        terminal_id="OVERSEER",
        payload={"modifier_id": modifier_id, "micromod_id": micromod_id},
    )
    await ledger.append(event)
    return {"status": "ok", "modifier_id": modifier_id, "micromod_id": micromod_id}
