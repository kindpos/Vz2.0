...
import httpx
import urllib.parse
import xml.etree.ElementTree as ET_diag
...
async def process_sale(
    ...
    _overage_as_tip = Decimal("0.00")
    if order_proj is not None:
        balance = order_proj.balance_due
...
async def process_cash_payment(
    ...
    rate = Decimal(str(settings.cash_discount_rate))
    if not existing_confirmed and request.payment_method == "cash" and Decimal("0") < rate < Decimal("1"):
        # Cap the discount so partial (seat-level) payments don't inflate
        # the discount to include unpaid seats' value.
        naive_discount = money_round(order.total - Decimal(str(request.amount)))
        max_discount = money_round(Decimal(str(request.amount)) * rate / (Decimal("1") - rate))
        cash_discount = max(Decimal("0"), min(naive_discount, max_discount))
        if cash_discount > Decimal("0"):
...
class RefundRequest(BaseModel):
    order_id: str
    payment_id: str
    amount: Optional[Decimal] = None  # None = full refund of original payment
