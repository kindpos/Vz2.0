...
class PrintContextBuilder:
    ...
    async def build_server_checkout_context(
        ...
        for order in server_orders:
            ...
            # ── Payment details ───────────────────────────────────────────────
            # PAYMENT_REFUNDED doesn't mutate the underlying payment, so
            # p.amount still reflects the original tender. Build a per-
            # payment refund map so we can net refunds out per method.
            refund_by_pid: dict[str, Decimal] = {}
            for refund in (order.refunds or []):
                pid = refund.get("payment_id")
                if pid:
                    refund_by_pid[pid] = refund_by_pid.get(pid, _ZERO) + Decimal(
                        str(refund.get("amount", 0) or 0)
                    )
            ...
            for p in (order.payments or []):
                ...
                amount -= refund_by_pid.get(p.payment_id, _ZERO)
                ...
                if p.method == "cash":
                    ...
                    tip_open = getattr(p, "tip_open", False) or (tip == Decimal("0.00"))
                    if tip_open:
                        open_tip_count += 1
        ...
        gross_tips = cc_tips_total + (declared_cash_tips or Decimal("0.00"))
        ...
        for e in all_events:
            ...
            if e.event_type == EventType.USER_LOGGED_IN and clock_in is None:
                # Use only the first clock-in of the session (ignore double-taps)
        ...
        # comps_total folds into discounts for the P&L identity (check_pnl_identity
        # only knows gross/voids/discounts/refunds). Tips partition uses the
        # server's declared cash tips vs. card tips captured on payments.
    ...
    async def build_sales_recap_context(
        ...
        for order_id in order_ids:
            ...
            # PAYMENT_REFUNDED doesn't mutate the underlying payment, so
            # p.amount still reflects the original tender. Net out refunds
            # against the method the refunded payment used, otherwise
            # Cash+Card won't match Net+Tax after any refund.
            for refund in (order.refunds or []):
                refund_amount = Decimal(str(refund.get("amount", 0) or 0))
                refund_pid = refund.get("payment_id")
                for p in (order.payments or []):
                    if p.payment_id == refund_pid:
                        if p.method == "cash":
                            cash_sales -= refund_amount
                        elif p.method == "card":
                            card_sales -= refund_amount
                        break
