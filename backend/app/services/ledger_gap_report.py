...
_CHECK_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-06", "check", "check.seat_added", "IMPLEMENTED", "HIGH",
          "Dark-shipped: per-seat add event (order_id + seat_number + optional seat_label + added_by). Emittable via /config/push. Coarse seats.updated still lands alongside for back-compat.",
          site="events.py:check_seat_added, config.py:push_changes",
          related_ids=["LG-07", "LG-08", "LG-47"]),
    _node("LG-07", "check", "check.seat_removed", "IMPLEMENTED", "HIGH",
          "Mirror of LG-06 (order_id + seat_number + removed_by + reason).",
          site="events.py:check_seat_removed, config.py:push_changes",
          related_ids=["LG-06"]),
    _node("LG-08", "check", "check.seat_relabeled", "IMPLEMENTED", "HIGH",
          "Per-seat rename event with previous_label + new_label + relabeled_by.",
          site="events.py:check_seat_relabeled, config.py:push_changes",
          related_ids=["LG-06"]),
    ...
    _node("LG-17", "check", "check.seat_sent_out", "IMPLEMENTED", "HIGH",
          "Dark-shipped 'send seat to other check' half of a cross-check seat transfer. target_order_id names the destination. Emittable via /config/push.",
          site="events.py:check_seat_sent_out, config.py:push_changes",
          related_ids=["LG-18"]),
    _node("LG-18", "check", "check.seat_received", "IMPLEMENTED", "HIGH",
          "Mirror of LG-17 on the destination check; carries source_order_id.",
          site="events.py:check_seat_received, config.py:push_changes",
          related_ids=["LG-17"]),
...
_SEAT_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-24", "seat", "seat.item_transferred_out", "IMPLEMENTED", "HIGH",
          "Dark-shipped item-level seat-to-seat transfer (target_seat_number + optional target_order_id for cross-check moves). Emittable via /config/push.",
          site="events.py:seat_item_transferred_out, config.py:push_changes",
          related_ids=["LG-25"]),
    _node("LG-25", "seat", "seat.item_received", "IMPLEMENTED", "HIGH",
          "Mirror of LG-24.",
          site="events.py:seat_item_received, config.py:push_changes",
          related_ids=["LG-24"]),
    ...
    _node("LG-37", "seat", "seat.paid", "IMPLEMENTED", "CRITICAL",
          ...
          site="payment_routes.py: process_cash_payment, process_sale"),
    _node("LG-38", "seat", "seat.transferred_out", "IMPLEMENTED", "HIGH",
          "Dark-shipped whole-seat transfer (all items + payments + tips as a unit) with target_order_id. Emittable via /config/push.",
          site="events.py:seat_transferred_out, config.py:push_changes",
          related_ids=["LG-39"]),
    _node("LG-39", "seat", "seat.transferred_in", "IMPLEMENTED", "HIGH",
          "Mirror of LG-38 with source_order_id.",
          site="events.py:seat_transferred_in, config.py:push_changes",
          related_ids=["LG-38"]),
    _node("LG-40", "seat", "seat.split_from", "IMPLEMENTED", "HIGH",
          "Per-seat split (parent_seat_number -> child_seat_number) so 'split the wine bill' is replayable without losing seat identity.",
          site="events.py:seat_split_from, config.py:push_changes"),
    _node("LG-41", "seat", "seat.merged_into", "IMPLEMENTED", "HIGH",
          "Mirror of LG-40 -- two seats combine into one.",
          site="events.py:seat_merged_into, config.py:push_changes"),
    _node("LG-42", "seat", "seat.reopened", "IMPLEMENTED", "MEDIUM",
          "Undoes seat.paid so post-close additions can land on a single seat without reopening the whole check.",
          site="events.py:seat_reopened, config.py:push_changes"),
