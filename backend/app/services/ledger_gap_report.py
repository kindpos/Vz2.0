...
_CHECK_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-19", "check", "check.day_locked", "IMPLEMENTED", "MEDIUM",
          "Per-order lock marker emittable in the close-day append_batch; payload carries the boundary event (day.closed) so post-lock edits can be flagged.",
          site="events.py:check_day_locked, config.py:push_changes"),
...
_SEAT_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-28", "seat", "seat.discount_applied", "IMPLEMENTED", "HIGH",
          "Per-seat discount with order_id + seat_number + amount + discount_type + optional discount_id / reason / approved_by. Emittable via /config/push alongside the order-level discount.approved.",
          site="events.py:seat_discount_applied, config.py:push_changes"),
    _node("LG-29", "seat", "seat.discount_voided", "IMPLEMENTED", "HIGH",
          "Reverses a seat.discount_applied on a specific seat. Emittable via /config/push.",
          site="events.py:seat_discount_voided, config.py:push_changes"),
    _node("LG-30", "seat", "seat.comped", "IMPLEMENTED", "HIGH",
          "Distinct from a generic discount.approved with type=comp; payload carries comp_category so comp-reporting can filter cleanly. Emittable via /config/push.",
          site="events.py:seat_comped, config.py:push_changes"),
    ...
    _node("LG-33", "seat", "seat.payment_voided", "IMPLEMENTED", "HIGH",
          "Seat-scoped payment void distinct from order-level payment.refunded; payload carries seat_number + payment_id + amount + voided_by. Emittable via /config/push.",
          site="events.py:seat_payment_voided, config.py:push_changes"),
...
_DAY_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-52", "day", "day.flash_report_generated", "IMPLEMENTED", "LOW",
          "Midday snapshot marker with report_id + window bounds + top-line total + generated_by. Emittable via /config/push.",
          site="events.py:day_flash_report_generated, config.py:push_changes"),
    _node("LG-53", "day", "day.closed", "IMPLEMENTED", "CRITICAL",
          ...
          site="orders.py:1781"),
    _node("LG-54", "day", "day.locked", "IMPLEMENTED", "MEDIUM",
          "Authoritative freeze marker (date + locked_by) emittable after day.closed so post-lock writes are distinguishable from in-window writes.",
          site="events.py:day_locked, config.py:push_changes"),
    _node("LG-55", "day", "day.reopened", "IMPLEMENTED", "LOW",
          "Reverses day.locked; high-friction (requires reopened_by and reason) so audit trail stays intact.",
          site="events.py:day_reopened, config.py:push_changes"),
]
...
_BATCH_NODES: list[dict[str, Any]] = [
    _node("LG-91", "batch", "batch.opened", "IMPLEMENTED", "HIGH",
          "Settlement-batch open marker (batch_id + opened_at + optional expected_close_at). Emittable via /config/push; correlation_id=batch_id so payments bin back to their batch cleanly.",
          site="events.py:batch_opened, config.py:push_changes"),
    _node("LG-92", "batch", "batch.settlement_initiated", "IMPLEMENTED", "HIGH",
          "Distinguishes 'sent to processor' from 'settled'; emittable via /config/push.",
          site="events.py:batch_settlement_initiated, config.py:push_changes"),
    ...
    _node("LG-95", "batch", "batch.reopened", "IMPLEMENTED", "MEDIUM",
          "Reverses a settled batch for next-day corrections; high-friction (reopened_by + reason required). Emittable via /config/push.",
          site="events.py:batch_reopened, config.py:push_changes"),
]
...
_TIPOUT_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-99", "tipout", "tipout.calculated", "IMPLEMENTED", "HIGH",
          "Tipout math result anchor: tipout_id + shift_date + total_tipout + rule_ids + per-recipient breakdown. Emittable via /config/push so 'why did I get $X' disputes are replayable.",
          site="events.py:tipout_calculated, config.py:push_changes"),
    _node("LG-100", "tipout", "tipout.adjusted", "IMPLEMENTED", "HIGH",
          "Manager override of a calculated tipout line (tipout_id + recipient_id + previous_amount + new_amount + adjusted_by). Emittable via /config/push.",
          site="events.py:tipout_adjusted, config.py:push_changes"),
    _node("LG-101", "tipout", "tipout.distributed", "IMPLEMENTED", "HIGH",
          "Pay-out anchor confirming the tipout cash was actually handed out. Emittable via /config/push; pairs with tipout.calculated on tipout_id.",
          site="events.py:tipout_distributed, config.py:push_changes"),
]
