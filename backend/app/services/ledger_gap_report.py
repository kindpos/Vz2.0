...
_CHECK_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-14", "check", "check.merged_into", "IMPLEMENTED", "HIGH",
          "Spec-equivalent: merge_orders emits check.merged with role=target on the target side. Entire merge (items, modifiers, source check.merged/order.voided, target check.merged) commits as one append_batch since Phase 1, so there is no longer a mid-loop double-count risk.",
          site="orders.py:merge_orders", related_ids=["LG-15"]),
    _node("LG-15", "check", "check.absorbed_by", "IMPLEMENTED", "HIGH",
          "Spec-equivalent: merge_orders emits check.merged with role=source on each source side. Same atomic batch as LG-14.",
          site="orders.py:merge_orders", related_ids=["LG-14"]),
    _node("LG-16", "check", "check.split_from", "IMPLEMENTED", "HIGH",
          "Spec-equivalent: split_by_seat emits check.split with role=parent on the parent and role=child on each child. Full fan-out (child order.created, item.added, modifier.applied, parent item.removed, directional check.split events) lands in one atomic append_batch since Phase 1.",
          site="orders.py:split_by_seat"),
