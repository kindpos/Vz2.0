...
_CHECK_NODES: list[dict[str, Any]] = [
    _node("LG-01", "check", "check.opened", "MISSING", "CRITICAL",
          "No dedicated open; order.created conflates order+check. Cannot reconstruct per-check open timestamp from ledger alone; projection must infer from first item-add.",
          related_ids=["LG-02"]),
...
_SEAT_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-37", "seat", "seat.paid", "MISSING", "CRITICAL",
          "No per-seat paid marker. 'Which seats still owe' is projection-only; recovery after crash is ambiguous."),
...
_DAY_NODES: list[dict[str, Any]] = [
    _node("LG-48", "day", "day.opened", "MISSING", "CRITICAL",
          "No explicit day-open; day begins implicitly on first order. Cannot distinguish 'day was never opened' from 'zero sales'; pre-open operations silently accepted."),
...
_STAFF_NODES: list[dict[str, Any]] = [
    ...
    _node("LG-59", "staff", "staff.pin_changed", "MISSING", "HIGH",
          "High severity: PIN rotations not ledgered; PCI/SOX audit gap."),
    ...
    _node("LG-61", "staff", "staff.reactivated", "MISSING", "MEDIUM",
          "Rehire events lost."),
    _node("LG-62", "staff", "clock.in", "RENAMED", "CRITICAL",
          "Renamed to user.logged_in. Semantic gap: login != clock-in. Tip-out and labor-cost reports over-counted for 'logged in but not on shift'. TOCTOU at staff.py:71 vs :88.",
          site="staff.py:88", related_ids=["LG-63"]),
    _node("LG-63", "staff", "clock.out", "RENAMED", "CRITICAL",
          "Mirror of LG-62 as user.logged_out.", site="staff.py:164", related_ids=["LG-62"]),
