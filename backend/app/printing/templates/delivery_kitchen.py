...
class DeliveryKitchenTicketTemplate(BaseTemplate):
    ...
    Kitchen-lineage template — prints on the Epson TM-U220 impact printer
    at 33 chars/line. BaseTemplate's default picks 42 when paper_width is
    80 (for thermal receipt printers), which is wrong for kitchen, so we
    force the kitchen width here unless the caller passes one explicitly.
    """

    def __init__(self, paper_width: int = 80, chars_per_line: int = 33):
        super().__init__(paper_width=paper_width, chars_per_line=chars_per_line)
