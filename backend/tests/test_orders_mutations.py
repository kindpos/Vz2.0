    ...
    for idx, item in enumerate(items):
        ...
        for m_idx, mod in enumerate(mods):
            await ledger.append(modifier_applied(
                ...
                modifier_price=Decimal(str(mod.get("price", "0.00"))),
            ))
    return order_id
...
class TestMergeOrders:
    ...
    async def test_merge_copies_items_and_voids_source(self, ledger):
        ...
        await _open_order_with_items(ledger, order_id="oT",
                                      items=[("Pizza", Decimal("15.00"), 1)])
        await _open_order_with_items(ledger, order_id="oS",
                                      items=[("Salad", Decimal("8.00"), 1)])
        ...
        assert res.subtotal == Decimal("23.00")
        # Source is voided
    ...
    async def test_merge_preserves_modifiers(self, ledger):
        ...
        await _open_order_with_items(ledger, order_id="oTM",
                                      items=[("Target", Decimal("5.00"), 1)])
        await _open_order_with_items(ledger, order_id="oSM",
                                      items=[("Pizza", Decimal("10.00"), 1,
                                              [{"name": "Extra cheese", "price": Decimal("3.00")}])])
        ...
        # $5 + ($10 + $3 mod) = $18
        assert res.subtotal == Decimal("18.00")

    @pytest.mark.asyncio
    async def test_merge_multiple_sources(self, ledger):
        await _open_order_with_items(ledger, order_id="oTa", items=[("T", Decimal("1.00"), 1)])
        await _open_order_with_items(ledger, order_id="oS1", items=[("S1", Decimal("2.00"), 1)])
        await _open_order_with_items(ledger, order_id="oS2", items=[("S2", Decimal("3.00"), 1)])
    ...
    async def test_merge_requires_manager_approval(self, ledger):
        await _open_order_with_items(ledger, order_id="oT1", items=[("T", Decimal("1.00"), 1)])
        await _open_order_with_items(ledger, order_id="oS1x", items=[("S", Decimal("2.00"), 1)])
        with pytest.raises(HTTPException) as exc:
            await orders_mod.merge_orders(
                "oT1",
    ...
    async def test_merge_rejects_closed_source(self, ledger):
        ...
        await _open_order_with_items(ledger, order_id="oT2", items=[("T", Decimal("1.00"), 1)])
        await _open_order_with_items(ledger, order_id="oSc", items=[("S", Decimal("2.00"), 1)])
        # Close the source
        ...
        await ledger.append(payment_confirmed(
            terminal_id=TERMINAL, order_id="oSc", payment_id="pSc",
            transaction_id="txnSc", amount=Decimal("2.00"), tax=Decimal("0.00"),
        ))
        await ledger.append(order_closed(
            terminal_id=TERMINAL, order_id="oSc", total=Decimal("2.00"),
        ))
    ...
    async def test_merge_rejects_source_with_confirmed_payment(self, ledger):
        ...
        await _open_order_with_items(ledger, order_id="oT3", items=[("T", Decimal("1.00"), 1)])
        await _open_order_with_items(ledger, order_id="oSp", items=[("S", Decimal("10.00"), 1)])
        ...
        await ledger.append(payment_confirmed(
            terminal_id=TERMINAL, order_id="oSp", payment_id="pSp",
            transaction_id="txnSp", amount=Decimal("10.00"), tax=Decimal("0.00"),
        ))
    ...
    async def test_merge_target_must_be_open(self, ledger):
        ...
        await _open_order_with_items(ledger, order_id="oTc", items=[("T", Decimal("5.00"), 1)])
        await ledger.append(payment_initiated(
        ...
        await ledger.append(payment_confirmed(
            terminal_id=TERMINAL, order_id="oTc", payment_id="p_close",
            transaction_id="txnTc", amount=Decimal("5.00"), tax=Decimal("0.00"),
        ))
        await ledger.append(order_closed(
            terminal_id=TERMINAL, order_id="oTc", total=Decimal("5.00"),
        ))
        await _open_order_with_items(ledger, order_id="oSok", items=[("S", Decimal("1.00"), 1)])
...
class TestApplyDiscount:
    ...
    async def test_cannot_discount_closed_order(self, ledger):
        ...
        await ledger.append(payment_confirmed(
            terminal_id=TERMINAL, order_id="oDc", payment_id="p",
            transaction_id="txnDc", amount=Decimal("10.00"), tax=Decimal("0.00"),
        ))
        await ledger.append(order_closed(
...
class TestSplitBySeat:
    ...
    async def test_splits_items_into_per_seat_child_orders(self, ledger):
        ...
        await _open_order_with_items(
            ...
            items=[("A", Decimal("10.00"), 1), ("B", Decimal("20.00"), 1)],
            seats=[1, 2],
        )
        ...
        assert child1.subtotal == Decimal("10.00")
        assert child2.subtotal == Decimal("20.00")
    ...
    async def test_modifiers_carry_over_to_split_children(self, ledger):
        ...
        await _open_order_with_items(
            ...
            items=[("Pizza", Decimal("15.00"), 1, [{"name": "Extra cheese", "price": Decimal("2.50")}])],
            seats=[3],
        )
        res = await orders_mod.split_by_seat(
            "oPmod",
        ...
        assert child.subtotal == Decimal("17.50")  # $15 + $2.50 modifier
    ...
    async def test_split_specific_seats_only(self, ledger):
        ...
        await _open_order_with_items(
            ...
            items=[("A", Decimal("5.00"), 1), ("B", Decimal("8.00"), 1)],
            seats=[1, 2],
        )
        res = await orders_mod.split_by_seat(
            "oPselect",
        ...
        assert parent.subtotal == Decimal("8.00")
    ...
    async def test_no_seated_items_400s(self, ledger):
        ...
        await _open_order_with_items(
            ...
            items=[("A", Decimal("5.00"), 1)],   # no seats assigned
        )
        with pytest.raises(HTTPException) as exc:
    ...
    async def test_cannot_split_closed_order(self, ledger):
        ...
        await ledger.append(payment_confirmed(
            terminal_id=TERMINAL, order_id="oPc", payment_id="p",
            transaction_id="txnPc", amount=Decimal("10.00"), tax=Decimal("0.00"),
        ))
        await ledger.append(order_closed(
            terminal_id=TERMINAL, order_id="oPc", total=Decimal("10.00"),
        ))
        with pytest.raises(HTTPException) as exc:
            await orders_mod.split_by_seat(
                "oPc",
...
class TestSplitEvenly:
    ...
    async def test_clean_division_three_ways(self, ledger):
        ...
        await _open_order_with_items(ledger, order_id="oE30",
                                      items=[("X", Decimal("30.00"), 1)])
        res = await orders_mod.split_evenly(
            "oE30",
            orders_mod.SplitEvenlyRequest(num_ways=3),
            ledger=ledger,
        )
        assert res["per_person"] == Decimal("10.00")
        assert res["last_person"] == Decimal("10.00")
...
class TestAdd86Guard:
    ...
    async def test_adding_restored_item_succeeds(self, ledger):
        ...
        assert res.subtotal == Decimal("4.00")
    ...
    async def test_ad_hoc_item_not_in_menu_still_added(self, ledger):
        ...
        assert res.subtotal == Decimal("0.50")
    ...
    async def test_available_item_still_added_normally(self, ledger):
        ...
        assert res.subtotal == Decimal("15.00")
