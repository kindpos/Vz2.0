...
def _zero_config(monkeypatch):
    ...
    monkeypatch.setattr(settings, "tipout_percent", Decimal("0.00"))
...
async def _seed_employee(ledger, eid: str, name: str, hourly_rate: Decimal):
...
async def _order_closed_with_tip(
    ledger, *, order_id: str, server_id: str, amount: Decimal, tip: Decimal = Decimal("0.00"),
):
    ...
    await ledger.append(payment_confirmed(
        terminal_id=TERMINAL, order_id=order_id, payment_id=pid,
        transaction_id=f"txn_{order_id}", amount=amount, tax=Decimal("0.00"),
    ))
    if tip > 0:
...
class TestLaborSummaryManagerView:
    ...
    async def test_wage_math_with_hourly_rate(self, ledger):
        ...
        assert emp["hourly_rate"] == Decimal("18.00")
        assert emp["gross_pay"] == money_round(emp["hours"] * Decimal("18.00"))
        assert res["total_labor"] == emp["gross_pay"]
    ...
    async def test_missing_rate_uses_zero_not_fabricated_default(self, ledger):
        ...
        assert emp["hourly_rate"] == Decimal("0.00")
        assert emp["gross_pay"] == Decimal("0.00")
    ...
    async def test_net_sales_surfaces_alongside_labor(self, ledger):
        ...
        await _order_closed_with_tip(
            ledger, order_id="oE1", server_id="emp_E", amount=Decimal("42.00"),
        )

        res = await get_labor_summary(date=TODAY, server_id=None, ledger=ledger)
        assert res["net_sales"] == Decimal("42.00")
    ...
    async def test_tips_attributed_to_server(self, ledger):
        ...
        await _order_closed_with_tip(
            ...
            amount=Decimal("20.00"), tip=Decimal("3.00"),
        )

        res = await get_labor_summary(date=TODAY, server_id=None, ledger=ledger)

        emp = next(e for e in res["employees"] if e["id"] == "emp_F")
        assert emp["tips"] == Decimal("3.00")
        assert res["card_tips_total"] == Decimal("3.00")
    ...
    async def test_empty_day_has_no_employees_no_crash(self, ledger):
        ...
        assert res["total_hours"] == Decimal("0.00")
        assert res["total_labor"] == Decimal("0.00")
...
class TestLaborSummaryServerView:
    ...
    async def test_ot_buffer_and_status_under_warning_threshold(self, ledger):
        ...
        assert float(res["today_hours"]) >= 0.9
        assert res["ot_status"] == "ok"
        assert float(res["ot_buffer"]) == pytest.approx(40.0 - float(res["weekly_hours"]), abs=0.1)
