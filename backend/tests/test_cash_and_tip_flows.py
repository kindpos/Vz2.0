...
async def test_cash_payment_success(ledger):
    ...
    assert Decimal(str(initiated[0].payload["amount"])) == Decimal("25.50")
    assert Decimal(str(confirmed[0].payload["amount"])) == Decimal("25.50")
...
async def test_cash_payment_auto_closes_order(ledger):
    ...
    request = CashPaymentRequest(order_id=order_id, amount=Decimal("16.05"))
