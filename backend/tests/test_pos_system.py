...
async def _add_item(ledger, order_id, item_id, name, price,
    ...
    evt = item_added(
        ...
        name=name, price=Decimal(str(price)), quantity=quantity, category=category,
    )
    await ledger.append(evt)
...
async def _pay(ledger, order_id, payment_id, amount, method="card"):
    init = payment_initiated(
        terminal_id=TERMINAL, order_id=order_id,
        payment_id=payment_id, amount=Decimal(str(amount)), method=method,
    )
...
class TestFinancialAccuracy:
    ...
    async def test_tax_after_flat_discount(self, ledger):
        ...
        assert order.tax == money_round(Decimal("45.00") * TAX_RATE)  # 3.15
        assert order.total == money_round(Decimal("50.00") - Decimal("5.00") + Decimal("45.00") * TAX_RATE)  # 48.15
    ...
    async def test_tax_rounding_half_up(self, ledger):
        ...
        order = await _get_order(ledger, oid)
        assert order.tax == money_round(Decimal("10.05") * TAX_RATE)  # 0.70
        assert order.total == money_round(Decimal("10.05") + Decimal("10.05") * TAX_RATE)  # 10.75
    ...
    @pytest.mark.asyncio
    async def test_size_based_pricing_and_included_modifiers(self, ledger):
        """
        Test that modifiers reprice correctly based on size (price_by_option)
        and that included modifiers are correctly handled in projections.
        """
        order_id = _oid()
        await _create_order(ledger, order_id)

        # 1. Add a Large Pizza ($20)
        item_id = "piz-1"
        await _add_item(ledger, order_id, item_id, "Large Pizza", 20.00)

        # 2. Apply Pepperoni modifier with size-based pricing
        # Resolved by frontend to $3.50 for Large
        mod_evt = modifier_applied(
            terminal_id=TERMINAL,
            order_id=order_id,
            item_id=item_id,
            ...
            modifier_price=Decimal("3.50"),
            quantity=1,
        )
        await ledger.append(mod_evt)

        # 3. Add an included modifier (Cheese) at $0
        mod_evt_cheese = modifier_applied(
            terminal_id=TERMINAL,
            order_id=order_id,
            item_id=item_id,
        ...
        await ledger.append(mod_evt_cheese)

        # 4. Project and verify
        events = await ledger.get_events_by_correlation(order_id)
        order = project_order(events, tax_rate=0)
        
        assert order.subtotal == Decimal("23.50")
        assert order.total == Decimal("23.50")
        
        # 5. Add a Medium Pizza ($15) with Pepperoni ($2.50 override)
        item_id2 = "piz-2"
        await _add_item(ledger, order_id, item_id2, "Medium Pizza", 15.00)
        ...
        # Large ($23.50) + Medium ($17.50) = $41.00
        assert order.subtotal == Decimal("41.00")
...
class TestTransactionFlow:
    ...
    async def test_reopen_after_close(self, ledger):
        ...
        await _create_order(ledger, oid)
        await _add_item(ledger, oid, "i1", "Burger", 10.00)
        await _close_order(ledger, oid, Decimal("10.70"))
        order = await _get_order(ledger, oid)
        assert order.status == "closed"
    ...
    async def test_refund_full_payment(self, ledger):
        ...
        assert result["refund_amount"] == Decimal("10.70")
    ...
    async def test_double_close_idempotent(self, ledger):
        ...
        await _close_order(ledger, oid, Decimal("10.70"))
        await _close_order(ledger, oid, Decimal("10.70"))  # second close
        order = await _get_order(ledger, oid)
        assert order.status == "closed"
        assert order.total == Decimal("10.70")
...
class TestReportingReconciliation:
    ...
    async def test_batch_submitted_totals_match(self, ledger):
        ...
        assert len(events) == 1
        assert Decimal(str(events[0].payload["total_amount"])) == total_sales
        assert Decimal(str(events[0].payload["cash_total"])) == order1.total
        assert Decimal(str(events[0].payload["card_total"])) == order2.total
