...
async def test_add_items_and_send(client, ledger):
    ...
    assert float(resp.json()["items"][0]["subtotal"]) == 12.50
    ...
    # 12.50 + 5.00*2 = 22.50
    assert float(data["subtotal"]) == 22.50
    # Tax at 0.07: 22.50 * 0.07 = 1.575 -> 1.58
    assert float(data["tax"]) == 1.58
    assert float(data["total"]) == 24.08
...
async def test_full_order_lifecycle(client, ledger):
    ...
    assert float(total) == 16.05  # 15.00 * 1.07
    ...
    assert resp.status_code == 200
    assert float(resp.json()["amount_paid"]) == float(total)
    assert float(resp.json()["balance_due"]) == 0.0
...
async def test_modify_item(client):
    ...
    assert float(resp.json()["subtotal"]) == 30.00
...
async def test_apply_modifier(client):
    ...
    assert float(item["subtotal"]) == 12.00  # 10.00 + 2.00
...
async def test_cash_payment_route(client, ledger):
    ...
    assert float(data["amount"]) == 21.40
...
async def test_tip_adjust_route(client, ledger):
    ...
    data = resp.json()
    assert data["success"] is True
    assert float(data["tip_amount"]) == 5.50
    assert float(data["previous_tip"]) == 0.0
...
async def test_financial_2dp_precision(client, ledger):
    ...
    assert float(data["subtotal"]) == 29.97
    # Tax: 29.97 * 0.07 = 2.0979 -> 2.10
    assert float(data["tax"]) == 2.10
    # Total: 29.97 + 2.10 = 32.07
    assert float(data["total"]) == 32.07
