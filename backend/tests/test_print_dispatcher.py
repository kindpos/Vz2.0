...
until a kitchen ticket never comes out. Tests cover:
  ...
  - _render: dispatches to receipt vs kitchen formatter by printer_type
             and raises cleanly on unknown template_id
  - _resolve_printer: fallback keys, hardware_config.db lookup, type-based
...
class TestRender:
    ...
    def test_receipt_template_rendered_with_receipt_formatter(self, dispatcher):
        ...
        out = dispatcher._render("guest_receipt", ctx, printer_type="receipt")
        assert isinstance(out, bytes)
        assert len(out) > 0

    def test_kitchen_template_rendered_with_kitchen_formatter(self, dispatcher):
        """kitchen_ticket + printer_type='kitchen' → bytes produced."""
        ctx = {
            "order_id": "O1",
            "items": [{"qty": 1, "name": "Burger"}],
            "fired_at": "2026-04-17T18:00:00+00:00",
        }
        out = dispatcher._render("kitchen_ticket", ctx, printer_type="kitchen")
        assert isinstance(out, bytes)
        assert len(out) > 0

    def test_unknown_template_raises_value_error(self, dispatcher):
        with pytest.raises(ValueError) as exc:
            dispatcher._render("not_a_template", {}, printer_type="receipt")
    ...
    def test_cross_bucket_fallback(self, dispatcher):
        ...
        # kitchen_ticket lives in _templates_kitchen; request it as receipt
        # to exercise the "other" bucket lookup.
        ctx = {
            "order_id": "O1",
            "items": [{"qty": 1, "name": "X"}],
        }
        out = dispatcher._render("kitchen_ticket", ctx, printer_type="receipt")
        assert isinstance(out, bytes)
...
from app.printing.print_dispatcher import PRINTER_PORT
...
class TestResolvePrinter:
    ...
    async def test_db_lookup_returns_ip_port_type(self, dispatcher, tmp_path, monkeypatch):
        ...
        conn.execute(
            ...
            ("AA:BB:CC:DD:EE:FF", "10.0.0.42", 9100, "kitchen"),
        )
        conn.commit()
        conn.close()
        monkeypatch.setattr(pd_mod, "HARDWARE_DB_PATH", str(db_path))

        ip, port, ptype = await dispatcher._resolve_printer("AA:BB:CC:DD:EE:FF")
        assert ip == "10.0.0.42"
        assert port == 9100
        assert ptype == "kitchen"
    ...
    async def test_db_row_missing_ip_uses_type_fallback(self, dispatcher, tmp_path, monkeypatch):
        ...
        conn.execute(
            "INSERT INTO devices (mac, ip, port, type) VALUES (?, ?, ?, ?)",
            ("11:22:33:44:55:66", None, 9100, "kitchen"),
        )
        conn.commit()
        conn.close()
        monkeypatch.setattr(pd_mod, "HARDWARE_DB_PATH", str(db_path))
...
class TestProcessJob:
    ...
    async def test_already_past_max_attempts_short_circuits(self, dispatcher, monkeypatch):
        ...
        async def fake_send(ip: str, port: int, data: bytes):
            send_attempts.append((ip, port, data))
