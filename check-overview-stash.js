...
//  KINDpos Terminal ΓÇö check-overview  (Vz2.0)
...
//    Left:  210 px TotalsBar ΓÇö CHECK TOTAL in State 1, "S1 + S3 TOTAL"
//           (sum of selected seats) in State 2.
//    Right: State 1 ΓåÆ PRINT + VOID (long-press 550 ms) | divider |
//                      PAY + ADD ITEMS.
//           State 2 ΓåÆ PRINT SEATS + MANAGE (T.elec) | divider |
//                      PAY SEATS (w/ seat-name sub-label) + ADD ITEMS.
//           MANAGE  ΓåÆ MOVE / SPLIT / MERGE tool pills | divider |
//                      UNDO + RESET (long-press) + DONE.
//
//  DISC is behind the existing disc-pin interrupt ΓÇö reachable via
...
// ΓöÇΓöÇ Inject invisible scrollbar style ΓöÇΓöÇ
(function() {
...
function fmt(n) {
  ...
  // ("9.00" is common) ΓÇö the raw `n.toFixed(2)` path used to throw a
...
//  component purely visual ΓÇö prefix parsing, half bucketing,
...
function _adaptOrderForRecap(state) {
  ...
  for (var s = 0; s < state.seats.length; s++) {
    ...
    adaptedSeats.push({
      ...
      subtotal:   seatTotal(seat),
      items:      adaptedItems,
      _sIdx:      s,  // original state.seats index ΓÇö used by the sort below
    });
  }
...
defineScene({
  ...
  state: {
    ...
    focusedSeats:  {},
    expandedSeats: {},
    _itemDiscounts: {},   // item_id ΓåÆ {pct, amount} ΓÇö survives refreshOrder
    _voidedItems:   [],   // {seatNumber, item} re-injected after every refreshOrder
    _seatDiscounts: {},   // seat.id  ΓåÆ {pct, amount} ΓÇö fallback when item_id absent
  },
  ...
  render: function(container, params, state) {
    ...
    state._tileSelSet   = null;       // legacy ΓÇö superseded by focusedSeats
    state.focusedSeats  = {};         // { seatId: true } ΓÇö Mode B filter
    state.expandedSeats = {};         // { seatId: true } ΓÇö Mode B collapsible
    ...
    if (!_landing) {
      entReport({
        ...
        message: 'returnLanding missing ΓÇö defaulting to server-landing',
        ctx: {
    ...
    // ΓöÇΓöÇ Root + body layout ΓöÇΓöÇ
    ...
    Object.assign(bottomRow.style, {
      ...
      display:       'flex',
      pointerEvents: 'auto',
      margin:        '0 -12px',   // break out of body padding ΓÇö matches header width
    });
    body.appendChild(bottomRow);
    state.bottomBarEl = bottomRow;

    // ΓöÇΓöÇ Initial paint ΓöÇΓöÇ
    state._params = params;
    renderActionBar(state);
    rerenderTopArea(state);

    // ΓöÇΓöÇ Fetch order ΓöÇΓöÇ
    if (state.orderId) {
  ...
  interrupts: {
    ...
    'co-item-menu': {
      render: function(container, params) {
        ...
        // Option pills ΓÇö scene passes a semantic opt.color (T.green,
...
//  Label and value shift with selection state: no seats ΓåÆ "CHECK
//  TOTAL" + full check total; seats selected ΓåÆ "S1 + S3 TOTAL" +
...
function renderActionBar(state) {
  ...
  // ΓöÇΓöÇ Selection-aware totals ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Left totals cluster ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Action grid ΓÇö 4 equal buttons ΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Print column: Print on top, Disc + Void sub-row on bottom ΓöÇΓöÇ
  ...
  // Sub-row builder ΓÇö shared press-state wiring
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  SEATS CONTAINER ΓÇö Nostalgia card shell
...
function renderSeatsGrid(state, container, mode) {
  ...
  // ΓöÇΓöÇ Mode B: collapsible recap LEFT + tile grid RIGHT ΓöÇΓöÇ
  if (mode === 'B') {
    ...
    // ΓöÇΓöÇ LEFT: recap shell ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    for (var rsi = 0; rsi < visibleSeats.length; rsi++) {
      ...
      // Items wrapper ΓÇö collapses/expands
    ...
    // ΓöÇΓöÇ RIGHT: 480px tile grid ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // ΓöÇΓöÇ Mode A: each seat is an equal flex-row column ΓöÇΓöÇ
  var activeCount = activeSeatCount(state.seats, state.paidSeats);
  for (var i = 0; i < state.seats.length; i++) {
...
// Accent is always T.moon (structural) ΓÇö selection state uses T.green.
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  ITEM BLOCK ΓÇö Nostalgia item card + mod tree
//  Shared between Mode A columns and Mode B recap.
//  modeB=true ΓåÆ sent item uses all-4-borders green;
//  modeB=false ΓåÆ sent uses left-border only.
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ

// Seat-level discount detection ΓÇö mirrors the item-level broadened check in
...
function buildItemBlock(state, seatIdx, itemIdx, modeB) {
  ...
  // the list price ΓÇö the backend may only surface the discount at the order level
  ...
  // ΓöÇΓöÇ Item card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // Void badge ΓÇö shown inline next to name
  if (isVoided) {
  ...
  // ΓöÇΓöÇ Discount row ΓÇö original price (struck) + savings amount ΓöÇΓöÇ
  if (isDisc) {
  ...
  // ΓöÇΓöÇ Mod tree ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  var mods = item.mods || [];
  ...
  if (mods.length > 0) {
    ...
    // Vertical stem ΓÇö always T.text
    ...
    for (var mi = 0; mi < mods.length; mi++) {
      ...
      // Horizontal branch ΓÇö always T.text
  ...
  // ΓöÇΓöÇ Sent-row wrapper (narrows block, adds >>> info) ΓÇö Mode A only ΓöÇΓöÇ
...
function _buildItemSubCard(state, seatIdx, itemIdx) {
  // Legacy shim ΓÇö delegates to buildItemBlock for any remaining callers.
...
=======
  ...
  return wrap;
>>>>>>> 3afc7648953272cd723f546fa631820d02e26452
}

// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  PAID SEAT HELPERS ΓÇö shared between Mode A cards and Mode B recap panel
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function _buildPaidRecapPanel(state, seatId) {
  ...
  titleEl.textContent = 'S' + seatNum + ' ΓÇö PAID';
...
// Paid seat card (Mode A) ΓÇö gold-infilled; body shows tappable payment rows.
function buildPaidSeatCard(state, seatIdx) {
  ...
  // ΓöÇΓöÇ Body: one tappable row per payment ΓöÇΓöÇ
  var body = document.createElement('div');
  Object.assign(body.style, {
    background:    hexToRgba(T.gold, 0.06),
    flex:          '1',
...
// Mode B compact tile ΓÇö header shows S# only (no subtotal column that
...
function buildCompactTile(state, seatIdx) {
  var seat = state.seats[seatIdx];
  // Tile focus uses focusedSeats (by seat ID) ΓÇö filter only, no payment selection.
  ...
  // Body: subtotal only ΓÇö no item count
  var body = document.createElement('div');
  Object.assign(body.style, {
    flex:           '1',
    display:        'flex',
...
// Paid compact tile (Mode B) ΓÇö gold-filled. Tapping selects it and
...
function buildAddTile(state, opts) {
  ...
  // cashier can add several seats in a single gesture ΓÇö handy for
...
function _wireItemTaps(state, seatIdx, itemIdx, el) {
  ...
  el.addEventListener('pointerdown', function() {
    ...
    lpTimer = setTimeout(function() {
      didLongPress = true;
      // If something is already selected ΓåÆ bulk menu
...
// ΓöÇΓöÇ Selection model ΓöÇΓöÇ
// state.selectedItems (keyed "seatIdx:itemIdx") is the source of truth.
// state.selected (keyed seat.id) is a derived mirror ΓÇö true iff every
...
function _syncSelectedFromItems(state) {
  var next = {};
  // Carry empty-seat selections forward ΓÇö they have no items to
...
// item. Empty seats flip state.selected directly ΓÇö no items to
// mirror ΓÇö so the tile still serves as ADD ITEMS scope. Individual
...
function deleteSeat(state, seatId) {
  ...
  if (state.paidSeats && state.paidSeats[seatId]) {
    ...
    showToast('CanΓÇÖt remove a paid seat ΓÇö reopen the payment first', { bg: T.gold });
    return;
  }
  if (state.seats[seatIdx].items.length > 0) {
    showToast('Seat has items ΓÇö void them first', { bg: T.verm });
    return;
  }
...
function persistSeats(state) {
  ...
  // order-entry via buildOrderEntryParams ΓåÆ seatNumbers and are written
  ...
  var myChain = _prevChain.then(function() {
    ...
    return fetchWithTimeout('/api/v1/orders/' + state.orderId + '/seats', {
      ...
      .catch(function(err) {
        ...
        showToast('Seat update failed ΓÇö please try again', { bg: T.verm, duration: 3000 });
      });
  });
  ...
  // would recurse on an already-resolved promise ΓÇö an infinite microtask
...
function handleAddItems(state, params) {
  // Order creation deferred to order-entry's first SAVE/SEND ΓÇö no POST here.
...
async function _gotoOrderEntry(state, params) {
  ...
  // check and auto-assigns every new item to seat 1 ΓÇö the "seats
  ...
  // After the await, state.order may STILL be null ΓÇö refreshOrder only sets it
  ...
  if (state.orderId && !state.order) {
    showToast('CouldnΓÇÖt load check ΓÇö check network and try again', { bg: T.verm, duration: 2500 });
    entReport({
      code: 'UI-005',
...
function handlePay(state, params) {
  ...
  if (selectedIds.length === 0) {
    // No seats selected ΓÇö default to "pay whole check" (all non-paid seats
    // with items on them).
    for (var i = 0; i < state.seats.length; i++) {
  ...
  // Build the seat-totals summary the payment scene needs ΓÇö seat IDs,
  ...
  // Guard: all selected seats already paid ΓÇö nothing to charge.
  ...
  // Pre-seed totals from the seats we're about to pay ΓÇö not state.order.total.
...
function _voidItems(state, refs) {
  ...
  // Fire DELETEs immediately ΓÇö no undo window.
  ...
  Promise.all(deletes)
    ...
    .catch(function() {
      // DELETE(s) failed ΓÇö roll back the local void so the display
      ...
      rerenderTopArea(state);
      showToast('Void failed ΓÇö check connection', { bg: T.verm });
    });
}
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
//  DISCOUNT (manager PIN ΓåÆ % picker ΓåÆ apply)
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function handleDiscount(state) {
  ...
  // as a manager ΓÇö use the logged-in employee ID as approvedBy directly.
...
function _applyDiscount(state, pct, itemRefs, seatIds, approvedBy) {
  ...
  // the wire body. Previously this was inlined with a TODO ΓÇö discount
  ...
  fetchWithTimeout('/api/v1/orders/' + state.orderId + '/discount', {
    ...
    // at the order level ΓÇö it does not stamp effectivePrice on individual items).
    ...
    for (var _di = 0; _di < lines.length; _di++) {
      ...
      // Per-seat cache (keyed by seat.id ΓÇö more reliable fallback)
...
function handleItemAction(state, optId, seatIdx, itemIdx) {
  if (optId === 'void') {
    ...
    showToast('Reprint ΓÇö coming soon', { bg: T.gold });
  }
}
...
function handleSeatAction(state, optId, seatId) {
  ...
  if (optId === 'void') {
    ...
    showToast('Rename seat ΓÇö coming soon', { bg: T.gold });
  } else if (optId === 'merge') {
  ...
  SceneManager.interrupt('server-picker', {
    onConfirm: function(server) {
      fetchWithTimeout('/api/v1/orders/' + state.orderId, {
        ...
        // Network/offline ΓÇö without this the promise hangs forever and
        // the server sees no toast, no retry, no signal that the transfer
        // did NOT happen. Explicit catch so the op is recoverable.
        showToast('Transfer failed ΓÇö check connection', { bg: T.verm });
      });
    },
    onCancel: function() {},
    excludeId: null,
...
function _onOSItemTap(state, idx) {
  ...
  // For now stubbed ΓÇö selection in Mode C is driven by compact tiles.
}
  ...
  SceneManager.interrupt('seat-payment', {
    ...
    onConfirm: function(paymentId) {
      fetchWithTimeout(
        ...
        if (r.ok) {
          // Do not mutate paidSeats/seatPayments locally ΓÇö let refreshOrder
...
function refreshOrder(state, params) {
  ...
  // ITEMS ΓåÆ order-entry) can await the backend truth before they thread
  ...
  // overwrite state.seats with the backend's pre-mutation view ΓÇö it
  ...
  // 15s abort guard ΓÇö matches order-entry's send/recall fetches so a hung
  ...
  state._refreshPromise = fetchWithTimeout('/api/v1/orders/' + state.orderId, { cache: 'no-store' }, 15000)
    ...
    .then(function(order) {
      ...
      // loaded. One-shot ΓÇö clear the flag so subsequent refreshes don't
